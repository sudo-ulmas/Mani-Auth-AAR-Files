...
enum class PullyBrightness(val raw: Int) {
  LIGHT(0),
  DARK(1);

  companion object {
    fun ofRaw(raw: Int): PullyBrightness? {
      return values().firstOrNull { it.raw == raw }
    }
  }
}
...
data class HostInfo (
  ...
  val brightness: PullyBrightness? = null
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): HostInfo {
      ...
      val brightness = pigeonVar_list[8] as PullyBrightness?
      return HostInfo(paymentSystemId, locale, pinfl, phoneNumber, lastLogin, environment, residentType, designVariant, brightness)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
...
private open class ApiPigeonCodec : StandardMessageCodec() {
  override fun readValueOfType(type: Byte, buffer: ByteBuffer): Any? {
    return when (type) {
      ...
      132.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
          PullyBrightness.ofRaw(it.toInt())
        }
      }
      133.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
